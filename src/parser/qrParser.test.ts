...
import { parseQrPayload } from "./qrParser.js";
...
describe("parseQrPayload", () => {
  ...
  it("does not coerce a comma amount into a number", () => {
    const parsed = unwrap(parseQrPayload(buildSwissQrPayload({ amount: "12,50" })));
    assert.equal(parsed.amount.value, null);
  });

  it("rejects payloads that are not Swiss Payments Code", () => {
    const parsed = parseQrPayload("NOTSPC\n0200\n1");
  ...
  it("rejects a missing EPD trailer", () => {
    ...
    assert.equal(parsed.ok, false);
    if (!parsed.ok) {
      assert.match(parsed.error, /Trailer must be EPD/);
    }
  });
});
