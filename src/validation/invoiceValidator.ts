...
/** ISO 7064 mod-97 for IBAN and ISO 11649 (rearrange first 4 chars, A=10…). */
...
function issue(severity: ValidationSeverity, code: string, field: string, message: string): ValidationIssue {
  return { severity, code, field, message };
}

function addressIssues(
  prefix: string,
  party: InvoiceData["creditor"] | NonNullable<InvoiceData["debtor"]>,
): ValidationIssue[] {
...
export function validateInvoice(invoice: InvoiceData | null): ValidationResult {
  ...
  if (iban !== invoice.account) {
    issues.push(
      issue(
        ...
        "iban.spaces",
        "account",
        "IBAN contained spaces that were stripped for validation only; the stored value is the compact form from normalization.",
      ),
    );
  }
  ...
  if (invoice.debtor) {
    issues.push(...addressIssues("debtor", invoice.debtor));
  }
