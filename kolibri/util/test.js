...
export { TestSuite, total, failed, asyncTest, allAsyncTests, withAppender };
...
const asyncTest = (name, asyncCallback) => {
    const assert = Assert();
    const done = asyncCallback(assert) // returns a promise
        .catch( _ => {
    ...
    asyncPending.push(done);
};

const allAsyncTests = () => Promise.all(asyncPending);
